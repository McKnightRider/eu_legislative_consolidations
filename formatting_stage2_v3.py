...
Run:
    python formatting_stage2_v3.py L_2017168EN.01001201.xml.html Prospectus_Regulation_Stage2.docx
"""
...
def add_annexes(doc: Document, soup: BeautifulSoup) -> None:
    for annex in soup.find_all(id=re.compile(r"^anx_[IVXLCDM]+$")):
        ...
        hp = add_center_heading(doc, heading_text)
        ...
        for p in annex.find_all("p"):
            ...
            if not txt or txt == heading_text:
                continue
            if any(c in tag_classes(p) for c in ["oj-normal", "oj-enumeration-spacing", "oj-doc-ti", "oj-ti-section-1", "oj-ti-section-2"]):
                paras.append(p)
        # Important: pass the full sequence so standalone "A." is merged with the following text using a tab.
        emit_numbered_paragraph_sequence(doc, paras, annex_mode=True)
